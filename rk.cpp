...
vector<vector<result>> RapinKarp(vector<string> text, vector<string> pattern, long long & count_comparison){
    ...
    int n = text.size();
    for(int i = 0;++count_comparison &&i < pattern_size;i++){
        m[i] = pattern[i].size();
    }
    ...
    for(int i = 0;++count_comparison &&i < pattern_size;i++){
        for(int j = 0;++count_comparison &&j < m[i] - 1;j++){
            h[i] = (h[i]*d)%q;
        }
    }
    ...
    //Duyệt Ngang trước;
    for(int row = 0; ++count_comparison && row < n;row++){
        for(int i =0;++count_comparison && i < pattern_size;i++){
            t[i] = 0;
            for(int j = 0; ++count_comparison && j < m[i]; j++){
                t[i] = (d * t[i] + text[row][j]) % q;
            }
            for(int j = 0; ++count_comparison && j <= n - m[i];j++){
                if(++count_comparison && p[i] == t[i]){
                    bool match = true;
                    for(int x = 0; ++count_comparison && x < m[i];x++){
    ...
    //Duyệt hàng dọc
     for(int col = 0; ++count_comparison && col < n;col++){
        for(int i =0;++count_comparison && i < pattern_size;i++){
            t[i] = 0;
            for(int j = 0; ++count_comparison && j < m[i]; j++){
